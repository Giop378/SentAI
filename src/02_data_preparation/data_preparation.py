...
dataset
#%% md
# ## Applicazione Bag Of Words CountVectorizer
#%%
from sklearn.feature_extraction.text import CountVectorizer

# Inizializza il vettorizzatore Bag of Words.
...
# Applica CountVectorizer per trasformare il testo in una matrice sparsa
X_sparse = bow.fit_transform(dataset['text_final_no_wa'])

# Stampa la matrice sparsa: tipo e dimensioni
print("=== Matrice BoW (sparsa) ===")
print(X_sparse)
print("\nShape della matrice sparsa:", X_sparse.shape, "\n")

# Converte la matrice sparsa in una matrice densa per visualizzare i conteggi
X_dense = X_sparse.todense()
print("=== Matrice BoW (densa) ===")
print(X_dense, "\n")
...
doc_index = 0
doc_vector = X_dense[doc_index].tolist()[0]
print("=== Dettagli per il primo tweet ===")
print("Testo originale:", dataset['text_final_no_wa'].iloc[doc_index])
print("Vettore (conteggi per ciascun token):")
print(doc_vector, "\n")

# Stampa i token effettivamente presenti nel primo tweet e il relativo conteggio
print("Token presenti nel primo tweet:")
for token, idx in bow.vocabulary_.items():
...
for i in range(min(3, X_dense.shape[0])):
    doc_vector = X_dense[i].tolist()[0]
    print(f"\nTweet {i+1}:")
    print("Testo originale:", dataset['text_final_no_wa'].iloc[i])
    print("Token presenti e conteggi:")
    for token, idx in bow.vocabulary_.items():
        count = doc_vector[idx]
        if count > 0:
            print(f"  Token: '{token}', Count: {count}")
